...
class Dataset:
    def __init__(
        ...
        with open(self.destination_file_name, "wb") as f:
            result = self.download_bucket(self.bucket_name, source_name, f)
            print(result)
        print("***INITIALIZED FIRST SHARD***")
    ...
    async def download_blob_to_stream(self, bucket_name, source_blob_name, file_obj):
        async with aiohttp.ClientSession() as session:
            storage_client = Storage(session=session)
            bucket = storage_client.bucket(bucket_name)
            blob = bucket.blob(source_blob_name)
            await blob.download_to_file(file_obj)
            print(f"Downloaded blob {source_blob_name} to file-like object.")

        return file_obj

    async def download_bucket(self, bucket_name, source_name, f):
        while True:
            try:
                result = await self.download_blob_to_stream(bucket_name, source_name, f)
                print("Downloaded")
                return result
    ...
    async def load_next_shard(self, display: bool = False):
        source_blob_name_train = "train/edufineweb_train_"
        source_blob_name_val = "val/edufineweb_val_"
        size = self.train_size if self.train else self.val_size
        count = self.train_idx if self.train else self.val_idx
        ...
        if count <= size:
            ...
            data = np.load(self.destination_file_name)
            os.remove(self.destination_file_name)
            print(f'removed {self.destination_file_name} successfully')

            with open(self.destination_file_name, "wb") as f:
                result = await self.download_bucket(self.bucket_name, source_name, f)
                print(result)
