...
class Dataset:
    def __init__(
        ...
        with open(self.process_path, "wb") as f:
            result = self.download_bucket(self.bucket_name, self.data[self.shard_idx], f)
            self.shard_idx += 1
            print(result)
            print("Initalized first shard")
    ...
    def load_next_shard(self):
        ...
        thread1 = threading.Thread(target=download_next)
        thread1.start()

        thread2 = threading.Thread(target=process_prev)
        thread2.start()
        ...
        print(f'removed {self.download_path} successfully')
        os.rename(self.download_path, self.process_path)
