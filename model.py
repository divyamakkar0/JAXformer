...
class MoE(nn.Module):
    ...
    def scatter(
        ...
        position_in_expert = jnp.swapaxes(
            position_in_expert, 0, 1
        )  # (B*T, k, n_experts)
...
class Transformer(nn.Module):
    ...
    def __call__(
        ...
        *B, T = x.shape
        ...
        moe_stat = jax.tree.map(lambda *x: jnp.stack(x, axis=0), *moe_stat)
...
class shardedModel:
    ...
    def init_weights(self, key, mesh):
        ...
        assert self.cfg.blocks // layer_devices, (
            "Number of blocks must be divisible by number of devices"
        )
    ...
    def pipeline(
        ...
        for i in range(microbatches + layers - 1):
            ...
            state = state.at[0].set(
                jnp.where(device_idx == 0, inputs[batch_idx], state[0])
            )
        ...
        def slice_moe(x: Array) -> Array:
            def each_layer(layer_idx, x):
                return jax.lax.dynamic_slice_in_dim(
                    x, layers_per_device * device_idx + layer_idx, microbatches, axis=0
                )

            sliced_x = jax.vmap(each_layer, in_axes=(0, -2), out_axes=(-2))(
                jnp.arange(layers_per_device), x
            )
            return sliced_x
        ...
        moe_stat = {
            "tokens_per_expert": moe_stat["tokens_per_expert"].sum(
                axis=0
            ),  # (experts,)
    ...
    def param_count(self, params):
        ...
        active_params = jax.tree.reduce(lambda x, y: x + y, active_params_map, 0)
