...
class MLA(nn.Module):
    ...
    def __call__(self, x, cKV_cache=None, kRT_cache=None, train=True):
        ...
        if self.rope:
            ...
            qrt = rearrange(self.Wqr(x), 'B T (nh d) -> B nh T d', nh=self.n_heads, d=self.dhR)
        ...
        v_k = rearrange(self.W_uKV(cKVt), 'B T (nh d) -> B nh T d', nh=self.n_heads, d=2*self.dk)
        ...
        weights = jnp.einsum('B n T d, B n t d -> B n T t', q, k) * (1/ ((self.dk) ** 0.5))
