...
def main(config: config):
    ...
    def train_step(loss_fn, params, key, *args, **kwargs):
        loss_grads = jax.value_and_grad(loss_fn, argnums=0, has_aux=True)
        loss, grads = loss_grads(params, key, *args, **kwargs, train=True)
    ...
    def accumulate_grads(key, x, y, state):
        ...
        train_step_jit = lambda key, params, x, y: train_step(
            loss_fn, params, key, x, y
        )
        ...
        grads = jax.tree_map(lambda x: x / config.grad_step, grads)

        return grads, acc_metrics

    # train_step
    def train_step_device(state, x, y):
        updated_key, step_key = jax.random.split(state.rng)
        grads, step_metrics = accumulate_grads(step_key, x, y, state)
        grads = jax.tree.map(lambda g: jax.lax.pmean(g, axis_name="x"), grads)
        new_state = state.apply_gradients(grads=grads, rng=updated_key)
        step_metrics = jax.tree.map(
            lambda x: jax.lax.pmean(x, axis_name="x"), step_metrics
        )
    ...
    for current_step in range(init_step, total_steps):
        state_step, metrics_step = train_step_dp_fn(state, x, y)
        state = state_step
